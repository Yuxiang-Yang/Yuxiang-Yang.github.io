      ...
      <template v-for="(val, key) in settings" >
        ...
        <dd :key="key + 'b'">
          <div v-if="val == 'true' || val == 'false'">
            <label><el-radio v-model="settings[key]" label="true">是</el-radio></label>
            <label><el-radio v-model="settings[key]" label="false">否</el-radio></label>
          </div>
          <div v-else-if="/^[0-9]+$/.test(val)">
            <el-input style="width: 100px" size="small"  type="number" v-model="settings[key]"></el-input>
          </div>
          <div v-else>
            <el-input style="width: 300px" size="small"  type="text" v-model="settings[key]"></el-input>
          </div>
        </dd>
      </template>
    </dl>
    <el-button size="small" @click="changeGlobalOption">保存</el-button>
  </div>
</template>
